...
function updateTime(id, intakecCode) {
	plus.nativeUI.pickTime(function(e) {
		var d = e.date;
		var h = d.getHours() < 10 ? '0' + d.getHours() : d.getHours();
		var m = d.getMinutes() < 10 ? '0' + d.getMinutes() : d.getMinutes();
		...
		mui.ajax({
			...
			success: function(data) {
				...
				if(document.getElementById('currentDate')){
					document.getElementById('currentDate').innerHTML = tempTime;
				}
				mui.ajax({
					url: url,
					type: 'get',
					data: {
						msgBody: JSON.stringify({
							...
							OperateTime: tempTime + document.getElementById(id).value + ':00',
							OperateStatus: '1'
						}),
						msgHeader: JSON.stringify({
...
function completeTime(id) {
	if(document.getElementById(id).getAttribute('littleOver')) {
		return;
	}
	plus.nativeUI.pickTime(function(e) {
		var d = e.date;
